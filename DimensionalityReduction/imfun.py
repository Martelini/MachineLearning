...
class crop_image1(object):
    ...
    def run(self, image, img_type, cmap):
        if cmap is not None:
            if img_type == 2:
                image_c = cv2.applyColorMap(image, cmap)
            else:
...
def crop_image(image, show, cmap):
    '''Function to crop images using mouse
    
    [image2, points] = crop_image(image, show, cmap)
    ...
    [points, image3] = cropping_class.run(image, img_type, cmap)
    
    # If 'show = True', show the final image, with the chosen cropping rectang.
    if show == True:
        window_name2 = 'Result from crop'
...
def crop_multiple(images, cmap):
    ...
    [I00, points0] = crop_image(I[0,...], None, cmap)
...
class crop_poly_multiple1(object):
    ...
    # This function actually run the 'crop_multiple' function
    def run(self, image, points, img_type, cmap):
        if cmap is not None:
            if img_type == 'gray':
                image_c = cv2.applyColorMap(image, cmap)
            else:
        ...
        points1 = points1.reshape((-1, 1, 2))
        image3 = np.zeros(np.shape(image_c),np.uint8)
...
def crop_poly_multiple(images, cmap):
    ...
    for n in range(1,len(I)):
        ...
        [Itemp, points1] = crop_class.run(I[n], points,
                                                     img_type, cmap)
        I2.append(Itemp)
